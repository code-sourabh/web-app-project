...
const BlogSlider = () => {
  ...
  const nextSlide = () => {
    setCurrentSlide((prev) => (prev + 1) % t('slides', { returnObjects: true }).length);
  };

  const prevSlide = () => {
    setCurrentSlide((prev) => (prev - 1 + t('slides', { returnObjects: true }).length) % t('slides', { returnObjects: true }).length);
  };
  ...
  return (
    <div className="blog-slider">
      ...
      <div className="blog-slider__container">
        ...
        <div className="blog-slider__indicators">
          {t('slides', { returnObjects: true }).map((_, index) => (
            <button
              key={index}
              onClick={() => setCurrentSlide(index)}
              className={`blog-slider__indicator ${index === currentSlide ? 'active' : ''}`}
